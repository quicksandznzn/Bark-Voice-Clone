...
class CustomTokenizer(nn.Module):
    ...
    def save(self, path):
        ...
        data_from_model = Data(
            self.input_size, self.hidden_size, self.output_size, self.version
        )
    ...
    def load_from_checkpoint(path, map_location: MAP_LOCATION = None):
        ...
        with ZipFile(path) as model_zip:
            filesMatch = [
                file for file in model_zip.namelist() if file.endswith("/.info")
            ]
...
def auto_train(
    data_path, save_path="model.pth", load_model: str | None = None, save_epochs=1
):
    data_x, data_y = {}, {}
        ...
        for i in range(save_epochs):
            ...
            for i in range(max(len(data_x), len(data_y))):
                ...
                model_training.train_step(
                    torch.tensor(x).to(device), torch.tensor(y).to(device), j % 50 == 0
