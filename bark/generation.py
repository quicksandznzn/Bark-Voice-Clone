...
CACHE_DIR = os.path.join(
    os.getenv("XDG_CACHE_HOME", default_cache_dir), "serp", "bark_v0"
)
...
def _load_model(ckpt_path, device, use_small=False, model_type="text"):
    ...
    if model_type == "text":
        tokenizer = BertTokenizer.from_pretrained(
            "/data1/develop/zhangshuai/Bark-VC/models/bert-base-multilingual-cased"
        )
        return {
            "model": model,
            "tokenizer": tokenizer,
...
def load_model(
    use_gpu=True, use_small=False, force_reload=False, model_type="text", path=None
):
...
def preload_models(
    ...
    _ = load_model(
        ...
        force_reload=force_reload,
        path=path if fine_model_path is None else fine_model_path,
    )
